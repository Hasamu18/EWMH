...
namespace Users.Application.Handlers
{
    internal class AssignWorkerToLeaderHandler : IRequestHandler<AssignWorkerToLeaderCommand, (int, string)>
    {
        ...
        public async Task<(int, string)> Handle(AssignWorkerToLeaderCommand request, CancellationToken cancellationToken)
        {
            ...
            var getProcessingRequests = await _uow.RequestRepo.GetAsync(a => a.Status == 1 && a.LeaderId.Equals(getWorker[0].LeaderId), includeProperties:"RequestWorkers");
            var count = getProcessingRequests.SelectMany(s => s.RequestWorkers).Where(w => w.WorkerId == request.WorkerId).Count();
            if (count != 0)
                return (409, $"Nhân viên này vẫn còn {count} yêu cầu cần xử lý, cần hoàn thành trước khi chuyển giao");

            var getProcessingShippings = (await _uow.ShippingRepo.GetAsync(a => a.Status != 3 && (a.WorkerId ?? "").Equals(request.WorkerId))).ToList().Count;
            if (getProcessingShippings != 0)
                return (409, $"Nhân viên này vẫn còn {getProcessingShippings} đơn hàng cần vận chuyển, cần hoàn thành trước khi chuyển giao");
