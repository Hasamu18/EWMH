...
export default function CapturePostRepairEvidenceScreen() {
  ...
  const requestId = useSelector(
    (state: RootState) => state.capturePostRepairEvidenceScreen.requestId
  );
  const capturedImage = useSelector(
    (state: RootState) => state.capturePostRepairEvidenceScreen.imageUri
  );
  const isPreviewVisible = useSelector(
    (state: RootState) => state.capturePostRepairEvidenceScreen.isPreviewVisible
  );
  ...
  const handleCapture = async () => {
    try {
      ...
      const compresedPhoto = await CompressPhoto(photo, 1000, 1000, 0.5);
      dispatch(setImageUri(compresedPhoto.uri));
      dispatch(setPreviewVisibility(true));
      dispatch(addRepairCompletedImage(compresedPhoto.uri));
    } catch (error) {
      console.log(error);
    } finally {
  ...
  const goToHomePage = () => {
    dispatch(resetPostRepairEvidence());
